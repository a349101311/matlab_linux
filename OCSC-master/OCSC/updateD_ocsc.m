...
for i_d = 1:para.max_it_d
    ...
    RELTOL = 1e-3; 
    %REL
%      h.r_norm(i_d) = norm(d(:)-s(:)) / max(norm(d(:)),norm(s(:)));
%      h.s_norm(i_d) = norm(-(s(:)-sold(:))) / norm(y(:));      
%      h.eps_pri(i_d)=sqrt(d_length)*ABSTOL / max(norm(d(:)),norm(s(:))) +RELTOL;
%      h.eps_dual(i_d)=sqrt(d_length)*ABSTOL / (para.rho_Z * norm(y(:))) + RELTOL;
